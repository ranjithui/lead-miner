// LeadMiner backend — zero-dependency Node (>=18) scraping service.
// Serves the frontend AND exposes /api/find which does REAL work:
//   1. resolve the company's domain
//   2. crawl their public pages (/contact, /about, /team ...) for real emails
//   3. DuckDuckGo HTML search for articles/social mentioning the company
//   4. DNS MX lookup = honest "this domain can receive mail" signal
//   5. pattern guesses as a clearly-labelled fallback
//
// No API keys, no paid services. Limits are honest: many sites publish no
// emails, and social networks block bots — those come back empty, not faked.
...
const BRAVE_KEY = process.env.BRAVE_API_KEY || '';   // set this to use Brave Search; falls back to DuckDuckGo if empty
...
// ── tiny fetch wrapper: timeout + UA + only keep text/html ──────────────
...
// mailto: links are the most reliable — pull them explicitly too
...
// Calls are serialised through braveChain so we never exceed the rate limit.
let braveChain = Promise.resolve();
function braveSearch(query) {
  const run = braveChain.then(() => braveFetch(query));
  braveChain = run.then(() => sleep(1100), () => sleep(1100)); // space out next call either way
  return run;
}
...
// Unified search: Brave when a key is set (with DuckDuckGo as fallback), else DDG.
...
// ── DuckDuckGo HTML endpoint (no key) → list of result URLs ────────────
...
// ── resolve the most likely official domain for a company ──────────────
...
async function findLeads({ company, name, domain }) {
  ...
  if (!resolved) return { domain: null, mx: [], emails: [], sources: [], notes: ['Could not resolve a domain for this company.'] };
  ...
  const found = new Map(); // email -> {sourceUrl, where}
  const sources = [];
  ...
  for (const r of pageResults) {
    ...
    for (const e of emails) {
      // keep emails that belong to this company's domain, or any found on their own site
  ...
  // record the social/article links we saw, even if no email extracted
  ...
  // assemble: real found emails first, then labelled guesses
  const emails = [];
  ...
  if (name && name.trim()) {
    for (const g of patternGuesses(name, resolved, mxValid)) {
      if (found.has(g.email)) continue; // already confirmed real
      emails.push({
        ...g, type: 'person', domain: resolved, sourceCompany: company,
        sourceUrl: null, real: false,
      });
    }
  }
...
server.listen(PORT, () => {
  ...
  console.log('  Search: ' + (BRAVE_KEY ? 'Brave Search API ✓ (DuckDuckGo fallback)' : 'DuckDuckGo scraping (set BRAVE_API_KEY to use Brave)'));
